...
document.querySelectorAll('.category-dropdown .dropdown a').forEach(link => {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    // Function to load and display posts based on the category
    function loadPosts() {
        const posts = JSON.parse(localStorage.getItem('posts')) || [];
        ...
        if (filteredPosts.length === 0) {
            postContainer.innerHTML = '<p>No posts available for this category.</p>'; // Display message if no posts are found
            return;
        }
