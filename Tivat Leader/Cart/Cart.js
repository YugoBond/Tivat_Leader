...
document.addEventListener('DOMContentLoaded', function() {
  const cartContainer = document.getElementById('cartContainer'); // Ensure this ID matches your cart container
  loadCart();
  ...
  function loadCart() {
      ...
      if (cartItems.length === 0) {
          cartContainer.innerHTML = '<p>No items in cart.</p>'; // Display message if cart is empty
          return;
      }
      ...
      cartItems.forEach(item => {
          ...
          itemElement.classList.add('cart-item'); // Add any styling classes as needed
          ...
          itemElement.innerHTML = `
              ...
              <div class="item-details">
                  ...
                  <p>Total Price: <span class="total-price">${item.price}</span> ${item.currency}</p>
          ...
          itemElement.querySelector('.removeButton').addEventListener('click', function() {
              removeCartItem(item);
          });
      });
  }
  ...
  function removeCartItem(itemToRemove) {
      ...
      cartItems = cartItems.filter(item => item.title !== itemToRemove.title); // Remove the specific item
      localStorage.setItem('cartItems', JSON.stringify(cartItems)); // Update localStorage
      loadCart(); // Refresh the displayed cart items
